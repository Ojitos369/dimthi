...
export const localStates = () => {
    ...
    const [tipoMaterial, setTipoMaterial] = createState(['mjPerfiles', 'tipoMaterial'], 'filamento');
    ...
    const openNew = useCallback(() => {
        setEditId(null); setNombre(''); setTipoMaterial('filamento'); setFilamentoId(''); setResinaId(''); setMaquinaId('');
        setLuzKw(4.5); setManoObra(15); setMargenUtilidad(20); setShowForm(true);
    }, []);

    const openEdit = useCallback((item) => {
        setEditId(item.id); setNombre(item.nombre||'');
        setTipoMaterial(item.tipo_material||'filamento');
    ...
    }, [nombre, tipoMaterial, filamentoId, resinaId, maquinaId, luzKw, manoObra, margenUtilidad, editId, f.calculadora, cancel]);
