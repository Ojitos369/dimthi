...
export const localStates = () => {
    ...
    const openNew = useCallback(() => {
        setEditId(null); setNombre(''); setFilamentoId(''); setResinaId('');
        setLuzKw(4.5); setManoObra(15); setMargenUtilidad(20); setShowForm(true);
    }, []);

    const openEdit = useCallback((item) => {
        setEditId(item.id); setNombre(item.nombre||'');
        setFilamentoId(item.filamento_id||''); setResinaId(item.resina_id||'');
    ...
    }, [nombre, filamentoId, resinaId, luzKw, manoObra, margenUtilidad, editId, f.calculadora, cancel]);
