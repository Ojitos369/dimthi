...
export const RequestQuoteModal = ({ ls }) => {
    ...
    const handleSend = () => {
        submitPendingQuotes({ nombre, comentarios });
    };
    ...
    return (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', zIndex: 1100, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowRequestQuoteModal(false)}>
            <div style={{ background: '#111', padding: '0', borderRadius: '12px', width: '90%', maxWidth: '500px', color: 'white', overflow: 'hidden', border: '1px solid #333', boxShadow: '0 20px 50px rgba(0,0,0,0.8)' }} onClick={e => e.stopPropagation()}>
                <div style={{ padding: '20px', borderBottom: '1px solid #333', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <button style={{ background: 'transparent', border: 'none', color: '#888', fontSize: '1.5rem', cursor: 'pointer' }} onClick={() => setShowRequestQuoteModal(false)}>×</button>
                </div>
                ...
                <div style={{ padding: '20px', borderTop: '1px solid #333', display: 'flex', justifyContent: 'flex-end', gap: '10px', background: '#1a1a1a' }}>
                    <button style={{ padding: '10px 20px', borderRadius: '6px', border: '1px solid #444', background: 'transparent', color: '#ccc', cursor: 'pointer' }} onClick={() => setShowRequestQuoteModal(false)}>Cancelar</button>
                    <button 
                        style={{ padding: '10px 20px', borderRadius: '6px', border: 'none', background: '#7c3aed', color: 'white', fontWeight: 'bold', cursor: 'pointer' }} 
                        onClick={handleSend}
                        disabled={pendingCart.length === 0}
                    >
                        Enviar Solicitud
                    </button>
                </div>
